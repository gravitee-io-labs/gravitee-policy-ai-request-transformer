...
class AiRequestTransformerSchemaTest {
  ...
  void shouldExposeTransformerSettings() throws Exception {
    ...
    assertThat(schema.at("/properties/llm/type").asText()).isEqualTo("object");
    assertThat(
    ...
    assertThat(
      schema.at("/properties/maxRequestBodySize/default").asInt()
    ).isEqualTo(1048576);
    assertThat(
    ...
    assertThat(schema.at("/properties/errorMode/enum/0").asText()).isEqualTo(
      "FAIL_OPEN"
    );
    assertThat(schema.at("/properties/errorMode/enum/1").asText()).isEqualTo(
      "FAIL_CLOSED"
    );
